...
class Blake2b {
  ...
  Blake2b(int digestSize) {
    assert(digestSize == 160 || digestSize == 256 || digestSize == 384 || digestSize == 512);
  ...
  blake2b(final Int8List key, final int digestSize, final Int8List salt, final Int8List personalization) {
    this._buffer = Int8List(Const.blockLengthBytes);
  ...
  Int8List _buffer = Int8List(Const.blockLengthBytes);// new byte[BLOCK_LENGTH_BYTES];
  ...
  void init() {
    if (null == _chainValue) {
      ...
      // 2
      newChainValue.add(Const.blake2bIv[2]);// ^ ( nodeDepth | (innerHashLength <<
      // 8) );
  ...
  void update(Int8List message, int offset, int len) {
    ...
    if (0 != _bufferPos) { // commenced, incomplete buffer
  ...
  void _compress(Int8List message, int messagePos) {
    ...
    for (int round = 0; round < Const.rounds; round++) {
      _g(m[Const.blake2Sigma[round][0]], m[Const.blake2Sigma[round][1]], 0, 4, 8,
              12
      );
      _g(m[Const.blake2Sigma[round][2]], m[Const.blake2Sigma[round][3]], 1, 5, 9,
              13
      );
      _g(m[Const.blake2Sigma[round][4]], m[Const.blake2Sigma[round][5]], 2, 6, 10,
              14
      );
      _g(m[Const.blake2Sigma[round][6]], m[Const.blake2Sigma[round][7]], 3, 7, 11,
              15
      );
      _g(m[Const.blake2Sigma[round][8]], m[Const.blake2Sigma[round][9]], 0, 5, 10,
              15
      );
  ...
  void _g(Int64 m1, Int64 m2, int posA, int posB, int posC, int posD) {
    ...
    _internalState[posD] = ByteUtils.rotr64(
      _internalState[posD] ^ _internalState[posA],
      32
    );
  ...
  Int8List _arrayCopy(Int8List src, int srcOffset, Int8List dst, int dstOffset, int length) {
